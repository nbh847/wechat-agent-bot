...
Agent 始终从 `wechat-agent-bot` 项目路径运行；`project` 只是本次任务的目标项目，必须是 `/Users/mac/workspace` 下已有项目的相对路径。口语化消息明确提到一个项目时会把它作为本轮目标；未提项目时沿用默认目标项目；同时提到多个项目或无法识别目标时才询问。改变目标项目不会开启新 Agent 会话。上海时区跨自然日后的第一条任务、累计 30 次 Agent 调用后的下一条任务、显式“新任务／新会话”会开启新上下文；自动轮换只携带不超过 1000 字的最小交接摘要。跨项目写入及删除、Git 高风险操作、凭证、数据库、全局依赖、系统配置、Cloudflare 和公开发布等操作必须二次确认。配置合规执行端后，只读任务可直接执行；需要确认的任务只有在一次性授权通过后才会执行。未配置执行端时只记录任务，不执行操作。
